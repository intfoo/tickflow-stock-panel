...
export function Data() {
  ...
  // fork: ETF 可能独立配置 etf_data_provider (上游能力矩阵无 ETF 能力格)。
  // 独立配置时显示该源 (具备 etf/daily dataset), 否则跟随 daily 路由显示。
  const etfProviderDisplay = (): string | null => {
    const etfPref = prefs.data?.etf_data_provider
    if (etfPref && etfPref !== 'same_as_daily') {
      if (etfPref === 'tickflow') return null
      const ds = dataSources.data?.custom?.find(s => s.name === etfPref)
      return ds && (ds.datasets.includes('etf') || ds.datasets.includes('daily'))
        ? ds.display_name || etfPref
        : null
    }
    return routeProviderDisplay(matrix.data, 'daily')
  }
      ...
      case 'etf':
        return (
          <StatCard
            ...
            tierKey="etf"
            capLimits={mergedCaps}
            customProvider={etfProviderDisplay()}
            auto={etfAuto}
            subLabel="维表 · 日K · 指标"
            fieldTabs={[
