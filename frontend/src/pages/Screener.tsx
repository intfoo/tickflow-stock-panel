...
export function Screener() {
  ...
  const handleRun = (s: ScreenerStrategy) => {
    ...
    // ETF 的 asOf 已跟随 ETF enriched (见 activeEnriched), 显式传日期;
    // 分钟策略传空日期让后端用最新分钟分区 (与股票 enriched 可能不同日)。
  ...
  const handleDateChange = (newDate: string) => {
    ...
    if (assetType !== 'stock' && activeStrategy) {
      run.mutate({ id: activeStrategy, date: newDate, timeframe: activeStrategyTimeframe })
    }
  }
