...
const DATASET_LABEL: Record<string, string> = {
  realtime: '实时',
  daily: '日K',
  etf: 'ETF', // fork: ETF 复用 daily dataset, 独立 etf_data_provider 路由键
  minute: '分钟',
  adj_factor: '除权',
  depth5: '五档',
  financial: '财务',
...
function CapabilityRoutingSection() {
  ...
  const resetMut = useMutation({
    // fork: etf_data_provider 不在 DEFAULT_ROUTING 内, 恢复默认一并重置
    mutationFn: () => api.updateDataProviders({ ...DEFAULT_ROUTING, etf_data_provider: 'same_as_daily' }),
    onMutate: async () => {
      await qc.cancelQueries({ queryKey: QK.capabilityMatrix })
      await qc.cancelQueries({ queryKey: QK.preferences })
...
export function SettingsDataSourcesPanel({ highlight }: { highlight?: string } = {}) {
  ...
  const switchProvider = useMutation({
    mutationFn: async (name: string) => {
      ...
      if (name === 'tickflow') {
        // fork: etf_data_provider 不在上游 DEFAULT_ROUTING 内 (ETF 非注册能力), 重置时一并回 same_as_daily
        return api.updateDataProviders({ ...DEFAULT_ROUTING, etf_data_provider: 'same_as_daily' })
      }
