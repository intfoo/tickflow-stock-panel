...
export function EChartsCandlestick({
  ...
  const getInfoBarHTML = useCallback(() => {
    ...
    if (hoverActiveRef.current && prev && Number.isFinite(prev.close) && prev.close > 0) {
      ...
      if (Number.isFinite(latestPrice)) {
        ...
        const sinceClr = sinceRatio >= 0 ? CT().bull : CT().bear
  ...
  }, [data, markers, ranges, priceLines, linkedPrice, showMA, showMarkersProp, activeIndicators, volumeCompare, addedDate, chartHeight, dates, dateIndexMap, initialZoom, getInfoBarHTML, theme, colorMode])
