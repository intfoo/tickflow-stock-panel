...
export interface MonitorRule {
  ...
  webhook_channels?: string[]  // 命中时推送的外部渠道 (合法值 'feishu' | 'wecom' | 'wecom_bot' | 'custom' | 'email')
  created_at?: string
...
export const api = {
  ...
  // etf_data_provider 是 fork 私有路由键 (ETF 复用 daily dataset), 不在后端能力注册表内,
  // 故不进 ProviderField 联合类型, 在此显式并上
  updateDataProviders: (cfg: Partial<Pick<Preferences, ProviderField | 'etf_data_provider'>>) =>
    request<Pick<Preferences, ProviderField | 'etf_data_provider'>>(
  ...
  sendTestWebhook: (channel: 'feishu' | 'wecom' | 'wecom_bot' | 'custom' | 'email') =>
