...
def test_run_regime_batch_aggregates_fallback_per_batch(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        regime_builder,
        "_compute_batch",
        lambda repo, enriched, instruments, shares, start, end, warmup,
               part_files=None: _batch_frame(start, end),
    )
...
def test_scan_enriched_fallback_excludes_st_symbols(tmp_path, monkeypatch):
    """慢路径(fallback)的 ST 剔除: excluded_symbols 的行在逐批聚合前被过滤。

    与缓存路径(test_run_regime_batch_excludes_st)同语义: 过滤 symbol 后按日聚合
    ≡ 全量过滤后聚合。本用例直接对比传/不传 excluded_symbols 两种结果。
    """
    ...
    filtered = regime_builder._scan_enriched_fallback(
        repo, days[0], days[-1], index_pct_map={}, excluded_symbols=["STA"],
    )
