...
class GenericHTTPProvider:
    ...
    def validate(self) -> list[str]:
        ...
        for dataset, cfg in self.config.datasets.items():
            ...
            if dataset != "realtime":
                ...
                if dataset in {"minute", "full_minute", "daily", "etf", "adj_factor"}:
    ...
    def _request_rows_retry(
        ...
        **req_kwargs,
        ...
        req_kwargs 透传给 _request_rows (如 override_params/override_body)。
        """
        last: Exception | None = None
        for attempt in range(retries + 1):
            try:
                return self._request_rows(
                    cfg, symbols=symbols, start_time=start_time, end_time=end_time, **req_kwargs
                )
