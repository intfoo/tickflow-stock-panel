...
def get_adj_factor_provider() -> str:
    # 「跟随日K」(same_as_daily) 特殊值已下线: 新配置均为独立路由。
    # fork 兼容: 存量配置里的 same_as_daily 在 getter 层直接解析为日K源返回
    # (而非按非法值回退 tickflow) — fork 线上 adj=same_as_daily → 跟随
    # daily=amazingdata 供 ETF 因子, 回退 tickflow 会让无 ADJ_FACTOR 能力的
    # 档位静默断供 ETF 因子 (_can_sync_etf_adj=False)。
...
# 复盘推送渠道白名单 (飞书/企业微信群推送 Webhook + 第三方 Webhook + 邮件)。
# 多选: 不推送 = 空数组, 而非 'none'
PUSH_CHANNELS = {"feishu", "wecom", "custom", "email"}
REVIEW_PUSH_CHANNELS = PUSH_CHANNELS
# 监控规则推送渠道白名单 = 复盘渠道 + wecom_bot 智能机器人长连接。
# 注意与 REVIEW_PUSH_CHANNELS 区分: 复盘推送不支持 wecom_bot。
WEBHOOK_RULE_CHANNELS = PUSH_CHANNELS | {"wecom_bot"}
